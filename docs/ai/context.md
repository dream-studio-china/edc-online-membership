...
│                                 主套件 = UnitTest + Integration（2274 tests）；低价值可 --group low-value 显式运行
...
- **Test count**: **2220 tests / 7936 assertions** in the default suite (UnitTest + Integration), plus **477 low-value tests** excluded by default; 34 skipped tests document real `src/` bugs (see §23 and `docs/issues/coverage-2026-08-09/`); **never delete or un-skip them** without fixing the underlying bug
