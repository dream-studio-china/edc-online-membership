...
| `deductMemberPoints()` | 0703 `POST /api/vip.api?method=vipsubscore` | Deducts a positive integer number of member points synchronously, through the `score` field. |

0703 only deducts and only through `score`: `creditscore`/`debitscore` are ignored by the vendor, `dirflag` has no effect (a `"+"` call still subtracts), and crediting is impossible. There is therefore no credit method and no refund path anywhere in this bundle.

Every 0703 deduction is write-verified: the member balance is read via 0701 before and after the call, and a mismatch with the expected deduction throws instead of reporting success. This fail-closed check protects the payment gateway and manage adjustments against vendor responses that report success without applying the change.
...
Live testing shows that 0701 requires `method=getvipmember` in the query string and a JSON request body. The body accepts `mobile`, `cardid`, or `cloudId`; no matching member returns `code=501`, `msg=没有匹配到会员资料！`, and `data=null`, which the adapter maps to an empty list. The local member endpoint registers an empty 0701 result through 0702 and immediately returns its successful registration response. If 0702 fails and a re-query of 0701 still returns no member, the endpoint degrades to an empty profile (HTTP 200) instead of failing the read; the provider error is logged by the service. Registration uses the mobile as the vendor `name` and `alias`, with zero initial score and balances, and resolves `cardtypeName` from the Store's `getvipcardtype` list by the configured `memberCardTypeId` (best effort — an empty name is accepted by 0702). The payload mirrors the vendor's documented "021 会员资料数据结构" exactly (its 16 required string fields) plus `cardtypeId`, which the implementation demands although the document omits it. `expirydate` defaults to `2099-12-31` per operator instruction. Server-owned keys (`id`, `code` — the vendor generates a UUID id and an 8-digit code, verified live) and personally identifying fields the app does not own (`sex`, `birthday`, `salesman`, …) are left empty for the vendor, which stores them as null. 0702 requires a Store-specific `memberCardTypeId`.
...
| `GET` | `/api/v1/app/liansheng/member` | `ROLE_USER` | 0701 |
...
Every Liansheng endpoint requires `X-Store-Code`; it selects the Store-specific Liansheng configuration and token. App `/member` queries the current authenticated User's `phone`, never a client-provided mobile. The Manage 0703 endpoint accepts `mobile`, `dirflag` (`-` only — crediting is unsupported and rejected), positive integer `points`, stable `reference`, non-empty `roomtable`, and non-empty `remarks`, and delegates to the same Service used by the payment Gateway.
...
| Completion | Synchronous: one 0703 call returning vendor success plus a matching balance change produces `PaymentResult(status=paid)` |
| Retry | The gateway performs no internal retry |
| Notify | Unsupported because payment completes synchronously |
| Refund | Unsupported: the vendor 0703 API cannot credit points, so `refund()` throws instead of calling the provider |
...
The invoice must already use currency `LIANSHENG_POINT`, have a positive integer amount, and belong to a payer with a verified phone. A provider error, an unapplied deduction, or an ambiguous transport failure is propagated and never converted to a paid result. Reusing `outTradeNo` gives the provider a stable reference, but exactly-once behavior across an unknown network outcome still depends on provider-side deduplication of `accno`/`billno`.

Refunds are not supported and `refund()` throws `LogicException` without calling the provider, because crediting points through 0703 is impossible (a `"+"` call still subtracts).

The local test configuration uses `http://test.lincson.cn/web`. It completes 0102, 0701, 0703 (deduction only, via `score`), and 0704 with the supplied test application. This HTTP host is suitable only for local testing because credentials and tokens are transmitted in plaintext; production requires a vendor-provided HTTPS endpoint whose 0703 CRM Factory is configured for the Store.

0703 sends `mobile`, `accountdate`, `dirflag` (`-`), `score`, `expirydate`, `accno`, `billno`, `roomtable`, and `remarks` on every deduction. `creditscore`/`debitscore` are never sent because the vendor ignores them. `roomtable` is the non-empty `ONLINE` default and `remarks` falls back to `Liansheng points deduction` when the caller provides none.
