...
final class RegistryTestGateway implements PaymentGatewayInterface
{
    ...
    public function pay(Invoice $invoice, array $options = []): PaymentResult { return new PaymentResult($invoice, Invoice::STATUS_PAYING); }
    public function notify(Request $request): PaymentNotifyResult { return new PaymentNotifyResult('test', 'x', Invoice::STATUS_PAID, 1); }
    public function refund(Invoice $invoice, int $amount, string $reason, array $options = []): PaymentRefundResult { return new PaymentRefundResult($invoice, $amount, Invoice::STATUS_REFUNDED); }
