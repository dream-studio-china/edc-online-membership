...
class OpenApiEnricherListener
{
    ...
    private const META = [
        ...
        '/api/v1/app/store-orders/{id}' => ['summary' => ['get' => 'Get my store order detail'], 'desc' => ['get' => "Own StoreOrder only via customerUserUuid. Example: GET /api/v1/app/store-orders/1  Header: Authorization: Bearer <user_jwt> → {data:{uuid,tradeOrderUuid,operationalStatus}} or 404 if not own."]],
        ...
        '/api/v1/manage/stores/{id}' => ['summary' => ['get' => 'Get store detail', 'put' => 'Update store'], 'desc' => ['get' => "Example: GET /api/v1/manage/stores/1 → {data:{id,uuid,code,name,settings}}\nHeader: Authorization: Bearer <admin_jwt>", 'put' => "Example: PUT /api/v1/manage/stores/1\n{\"name\":\"Xuhui V2\",\"settings\":{\"fulfillment\":{\"requireVerification\":false}}} → 200"]],
        ...
        '/api/v1/store/{scopeId}/orders/{id}' => ['tag' => 'Store', 'summary' => ['get' => 'Get scoped store order detail'], 'desc' => ['get' => "Example: GET /api/v1/store/{storeUuid}/orders/1  Header: Authorization: Bearer <staff_jwt>\nRequires membership (store:order:read). → 200 {data:{uuid,tradeOrderUuid,operationalStatus,verificationRequired,verifiedAt}} or 404 if not in store scope."]],
        ...
        // --- Backfill: router paths missing META (auto-detected tag + fallback desc otherwise) ---
        '/api/v1/app/orders/{id}/refund' => ['summary' => ['post' => 'Refund own order'], 'desc' => ['post' => "Refund own order (must be own + workflow can refund). Body: reason required; if order has invoiceId → invoice refund path, else systemWalletId required.\nHeader: Authorization: Bearer <user_jwt>\nExample: POST /api/v1/app/orders/123/refund {\"reason\":\"changed mind\",\"systemWalletId\":1} → 200"]],
        '/api/v1/manage/order-items' => ['summary' => ['get' => 'List order items (admin)', 'post' => 'Create order item (admin)'], 'desc' => ['get' => "Paginated order-item list (snapshot rows). Supports @filter, @order. ROLE_ADMIN.\nHeader: Authorization: Bearer <admin_jwt>\nExample: GET /api/v1/manage/order-items?page=1&limit=20 → {data:[{id,uuid,specificationTitle,quantity,unitPrice,price}]}", 'post' => "Create order-item row directly. ROLE_ADMIN. Required: name, price (int cents). Optional: status, sort.\nHeader: Authorization: Bearer <admin_jwt>\nExample: POST /api/v1/manage/order-items {\"name\":\"128GB Silver\",\"price\":699900} → 201"]],
        '/api/v1/manage/order-items/batch-update' => ['summary' => ['post' => 'Batch update order items'], 'desc' => ['post' => "Batch upsert. Query: @mode=mixed|strict|create, @basis=id,uuid, @partial. Body: [{id,name,price}]\nHeader: Authorization: Bearer <admin_jwt>"]],
        '/api/v1/manage/order-items/{id}' => ['summary' => ['get' => 'Get order item detail', 'put' => 'Update order item', 'delete' => 'Delete order item'], 'desc' => ['get' => "GET /api/v1/manage/order-items/1 → {data:{id,specificationTitle,quantity,price}} ROLE_ADMIN.", 'put' => "Update name, price, status, sort. ROLE_ADMIN. Example: PUT /api/v1/manage/order-items/1 {\"price\":799900} → 200", 'delete' => "DELETE /api/v1/manage/order-items/1 → 200 ROLE_ADMIN."]],
        '/api/v1/manage/orders/{id}/status-reset' => ['summary' => ['put' => 'Reset order workflow status (admin)'], 'desc' => ['put' => "Admin escape hatch from WorkflowApiViewMixin: resets workflow marking. ROLE_ADMIN.\nHeader: Authorization: Bearer <admin_jwt>\nExample: PUT /api/v1/manage/orders/123/status-reset → 200"]],
        '/api/v1/manage/inventory/materials/batch-update' => ['summary' => ['post' => 'Batch update materials (admin)'], 'desc' => ['post' => "Batch upsert materials. Query: @mode=mixed|strict|create, @basis=id,code, @partial. Body: [{id|code,name,kind,unit,status}]\nHeader: Authorization: Bearer <admin_jwt>"]],
        '/api/v1/manage/inventory/materials/{id}' => ['summary' => ['get' => 'Get material detail', 'put' => 'Update material', 'delete' => 'Delete material'], 'desc' => ['get' => "GET /api/v1/manage/inventory/materials/1 → {data:{id,uuid,code,name}} ROLE_ADMIN.", 'put' => "Update name, kind, unit, status, metadata (code immutable after stock mutation). ROLE_ADMIN.", 'delete' => "DELETE /api/v1/manage/inventory/materials/1 → 200 ROLE_ADMIN."]],
        '/api/v1/manage/inventory/recipes/batch-update' => ['summary' => ['post' => 'Batch update recipes (admin)'], 'desc' => ['post' => "Batch upsert recipe status. Query: @mode, @basis=id, @partial. Body: [{id,status}]\nHeader: Authorization: Bearer <admin_jwt>"]],
        '/api/v1/manage/inventory/recipes/{id}' => ['summary' => ['get' => 'Get recipe detail', 'put' => 'Update recipe status', 'delete' => 'Delete recipe'], 'desc' => ['get' => "GET /api/v1/manage/inventory/recipes/1 → {data:{id,specificationUuid,lines}} ROLE_ADMIN.", 'put' => "Only status updatable (acceptedUpdateProperties: status). ROLE_ADMIN. Example: PUT /api/v1/manage/inventory/recipes/1 {\"status\":\"inactive\"} → 200", 'delete' => "DELETE /api/v1/manage/inventory/recipes/1 → 200 ROLE_ADMIN."]],
        '/api/v1/manage/inventory/stocks' => ['summary' => ['get' => 'List stock views (admin)'], 'desc' => ['get' => "Paginated stock views (storeUuid+materialUuid+onHand). Supports @filter. ROLE_ADMIN.\nHeader: Authorization: Bearer <admin_jwt>\nExample: GET /api/v1/manage/inventory/stocks?page=1&limit=20 → {data:[{storeUuid,materialUuid,onHand}]}"]],
        '/api/v1/manage/payment-deductions' => ['summary' => ['get' => 'List payment deductions (admin)'], 'desc' => ['get' => "Paginated payment-deduction ledger (all users). Supports @filter. ROLE_ADMIN.\nHeader: Authorization: Bearer <admin_jwt>\nExample: GET /api/v1/manage/payment-deductions?page=1&limit=20 → {data:[{id,amount,invoice:{id}}]}"]],
        '/api/v1/manage/payment-deductions/{id}' => ['summary' => ['get' => 'Get payment deduction detail (admin)'], 'desc' => ['get' => "GET /api/v1/manage/payment-deductions/1 → {data:{id,amount}} ROLE_ADMIN."]],
        '/api/v1/manage/profiles/batch-update' => ['summary' => ['post' => 'Batch update profiles (admin)'], 'desc' => ['post' => "Batch upsert profiles. Query: @mode, @basis=id, @partial. Body: [{id,level,nickname}]\nHeader: Authorization: Bearer <admin_jwt>"]],
        '/api/v1/manage/settlement-allocations' => ['summary' => ['get' => 'List settlement allocations (admin)'], 'desc' => ['get' => "Paginated allocation list (read-only, post/reverse via settlement-plans). Supports @filter. ROLE_ADMIN.\nHeader: Authorization: Bearer <admin_jwt>\nExample: GET /api/v1/manage/settlement-allocations?page=1&limit=20 → {data:[{uuid,plan:{uuid},amount,status}]}"]],
        '/api/v1/manage/settlement-allocations/{id}' => ['summary' => ['get' => 'Get settlement allocation detail'], 'desc' => ['get' => "GET /api/v1/manage/settlement-allocations/1 → {data:{uuid,amount,status}} ROLE_ADMIN."]],
        '/api/v1/manage/settlement-consumed-events' => ['summary' => ['get' => 'List consumed events (admin)'], 'desc' => ['get' => "Paginated idempotency/consumed-event log (read-only). Supports @filter. ROLE_ADMIN.\nHeader: Authorization: Bearer <admin_jwt>"]],
        '/api/v1/manage/settlement-consumed-events/{id}' => ['summary' => ['get' => 'Get consumed event detail'], 'desc' => ['get' => "GET /api/v1/manage/settlement-consumed-events/1 → {data:{id}} ROLE_ADMIN."]],
        '/api/v1/manage/settlement-outbox-messages' => ['summary' => ['get' => 'List outbox messages (admin)'], 'desc' => ['get' => "Paginated outbox log (read-only, relayed by scheduler). Supports @filter. ROLE_ADMIN.\nHeader: Authorization: Bearer <admin_jwt>"]],
        '/api/v1/manage/settlement-outbox-messages/{id}' => ['summary' => ['get' => 'Get outbox message detail'], 'desc' => ['get' => "GET /api/v1/manage/settlement-outbox-messages/1 → {data:{id}} ROLE_ADMIN."]],
        '/api/v1/manage/settlement-rule-versions/batch-update' => ['summary' => ['post' => 'Batch update rule versions (admin)'], 'desc' => ['post' => "Batch upsert rule versions. Query: @mode, @basis=id, @partial. Body: [{id,definition,priority}]\nHeader: Authorization: Bearer <admin_jwt>"]],
        '/api/v1/manage/store-orders/{id}' => ['summary' => ['get' => 'Get store order detail (admin)'], 'desc' => ['get' => "Admin detail by id (any store). Supports @select/@expands. ROLE_ADMIN.\nHeader: Authorization: Bearer <admin_jwt>\nExample: GET /api/v1/manage/store-orders/1 → {data:{uuid,tradeOrderUuid,operationalStatus}}"]],
        '/api/v1/manage/stores/batch-update' => ['summary' => ['post' => 'Batch update stores (admin)'], 'desc' => ['post' => "Batch upsert stores. Query: @mode, @basis=id,code, @partial. Body: [{id|code,name,timezone}]\nHeader: Authorization: Bearer <admin_jwt>"]],
        '/api/v1/manage/users/batch-update' => ['summary' => ['post' => 'Batch update users (admin)'], 'desc' => ['post' => "Batch upsert users. Query: @mode, @basis=id,email,username, @partial. Body: [{id,email,username}]\nHeader: Authorization: Bearer <admin_jwt>"]],
        '/api/v1/manage/voucher-comments' => ['summary' => ['get' => 'List voucher comments (admin)', 'post' => 'Create voucher comment (admin)'], 'desc' => ['get' => "Paginated voucher comments (all). Supports @filter. ROLE_ADMIN.\nHeader: Authorization: Bearer <admin_jwt>", 'post' => "Create voucher comment. ROLE_ADMIN. Required: voucher (id), text. Optional: actor auto-bound.\nHeader: Authorization: Bearer <admin_jwt>\nExample: POST /api/v1/manage/voucher-comments {\"voucher\":1,\"text\":\"Approved\"} → 201"]],
        '/api/v1/manage/voucher-comments/{id}' => ['summary' => ['get' => 'Get voucher comment detail (admin)'], 'desc' => ['get' => "GET /api/v1/manage/voucher-comments/1 → {data:{id,text}} ROLE_ADMIN. (No PUT/DELETE: admin controller is list+detail+create only.)"]],
        '/api/v1/manage/vouchers' => ['summary' => ['get' => 'List vouchers (admin)'], 'desc' => ['get' => "Paginated voucher ledger (all wallets). Supports @filter. ROLE_ADMIN. Mutations via /deposit, /withdraw, /{uuid}/reverse (not generic POST).\nHeader: Authorization: Bearer <admin_jwt>\nExample: GET /api/v1/manage/vouchers?page=1&limit=20 → {data:[{uuid,wallet:{id},amount,status}]}"]],
        '/api/v1/manage/vouchers/{id}' => ['summary' => ['get' => 'Get voucher detail (admin)'], 'desc' => ['get' => "GET /api/v1/manage/vouchers/1 → {data:{uuid,amount,status}} ROLE_ADMIN. Reverse via POST /api/v1/manage/vouchers/{uuid}/reverse."]],
        '/api/v1/manage/wechat-users' => ['summary' => ['get' => 'List WeChat users (admin)', 'post' => 'Create WeChat user (admin)'], 'desc' => ['get' => "Paginated WeChat bindings (all users). Supports @filter. ROLE_ADMIN.\nHeader: Authorization: Bearer <admin_jwt>", 'post' => "Create WeChat binding (admin). Body: openid/unionid/user refs. ROLE_ADMIN.\nHeader: Authorization: Bearer <admin_jwt>"]],
        '/api/v1/manage/wechat-users/batch-update' => ['summary' => ['post' => 'Batch update WeChat users (admin)'], 'desc' => ['post' => "Batch upsert. Query: @mode, @basis=id, @partial. ROLE_ADMIN.\nHeader: Authorization: Bearer <admin_jwt>"]],
        '/api/v1/manage/wechat-users/{id}' => ['summary' => ['get' => 'Get WeChat user detail', 'put' => 'Update WeChat user', 'delete' => 'Delete WeChat user'], 'desc' => ['get' => "GET /api/v1/manage/wechat-users/1 → {data:{id,openid}} ROLE_ADMIN.", 'put' => "Update binding fields. ROLE_ADMIN.", 'delete' => "DELETE /api/v1/manage/wechat-users/1 → 200 ROLE_ADMIN."]],
...
Errors: 401/403/404.']],
    ...
    private function centralRequestBody(string $path, string $method): ?array
    {
        ...
        return match ($key) {
            ...
            'post:/api/v1/app/orders/{id}/refund' => $inline(['reason'=>['type'=>'string','example'=>'changed mind'], 'systemWalletId'=>['type'=>'integer','example'=>1,'description'=>'Required when order has no invoice']], ['reason']),
            ...
            // ---- Backfill: order-items / stores / voucher-comments / wechat / batch ----
            'post:/api/v1/manage/order-items' => $inline(['name'=>['type'=>'string','example'=>'128GB Silver'], 'price'=>['type'=>'integer','example'=>699900,'description'=>'Price in cents'], 'status'=>['type'=>'string','enum'=>['active','inactive']], 'sort'=>['type'=>'integer']], ['name','price']),
            'put:/api/v1/manage/order-items/{id}' => $inline(['name'=>['type'=>'string'], 'price'=>['type'=>'integer'], 'status'=>['type'=>'string','enum'=>['active','inactive']], 'sort'=>['type'=>'integer']], [], false),
            'post:/api/v1/manage/order-items/batch-update' => ['required' => true, 'content' => ['application/json' => ['schema' => ['type' => 'array', 'items' => ['type' => 'object', 'properties' => ['id' => ['type' => 'integer'], 'name' => ['type' => 'string'], 'price' => ['type' => 'integer'], 'status' => ['type' => 'string'], 'sort' => ['type' => 'integer']]]]]]],
            'post:/api/v1/manage/stores' => $inline(['code'=>['type'=>'string','example'=>'xuhui'], 'name'=>['type'=>'string','example'=>'Xuhui Store'], 'timezone'=>['type'=>'string','example'=>'Asia/Shanghai'], 'currency'=>['type'=>'string','example'=>'CNY'], 'contact'=>['type'=>'object'], 'address'=>['type'=>'object'], 'settings'=>['type'=>'object']], ['code','name','timezone']),
            'put:/api/v1/manage/stores/{id}' => $inline(['name'=>['type'=>'string'], 'timezone'=>['type'=>'string'], 'currency'=>['type'=>'string'], 'contact'=>['type'=>'object'], 'address'=>['type'=>'object'], 'settings'=>['type'=>'object']], [], false),
            'post:/api/v1/manage/stores/batch-update' => ['required' => true, 'content' => ['application/json' => ['schema' => ['type' => 'array', 'items' => ['type' => 'object', 'properties' => ['id' => ['type' => 'integer'], 'code' => ['type' => 'string'], 'name' => ['type' => 'string'], 'timezone' => ['type' => 'string']]]]]]],
            'post:/api/v1/manage/voucher-comments' => $inline(['voucher'=>['type'=>'integer','example'=>1,'description'=>'Voucher id'], 'text'=>['type'=>'string','example'=>'Approved']], ['voucher','text']),
            'post:/api/v1/manage/users/batch-update' => ['required' => true, 'content' => ['application/json' => ['schema' => ['type' => 'array', 'items' => ['type' => 'object', 'properties' => ['id' => ['type' => 'integer'], 'email' => ['type' => 'string'], 'username' => ['type' => 'string']]]]]]],
            'post:/api/v1/manage/profiles/batch-update' => ['required' => true, 'content' => ['application/json' => ['schema' => ['type' => 'array', 'items' => ['type' => 'object', 'properties' => ['id' => ['type' => 'integer'], 'level' => ['type' => 'string'], 'nickname' => ['type' => 'string']]]]]]],
            'post:/api/v1/manage/inventory/materials/batch-update' => ['required' => true, 'content' => ['application/json' => ['schema' => ['type' => 'array', 'items' => ['type' => 'object', 'properties' => ['id' => ['type' => 'integer'], 'code' => ['type' => 'string'], 'name' => ['type' => 'string']]]]]]],
            'post:/api/v1/manage/inventory/recipes/batch-update' => ['required' => true, 'content' => ['application/json' => ['schema' => ['type' => 'array', 'items' => ['type' => 'object', 'properties' => ['id' => ['type' => 'integer'], 'status' => ['type' => 'string']]]]]]],
            'post:/api/v1/manage/settlement-rule-versions/batch-update' => ['required' => true, 'content' => ['application/json' => ['schema' => ['type' => 'array', 'items' => ['type' => 'object', 'properties' => ['id' => ['type' => 'integer'], 'definition' => ['type' => 'object'], 'priority' => ['type' => 'integer']]]]]]],
            // ---- Manage: products ----
            ...
            'post:/api/v1/manage/inventory/materials' => [
                ...
                'content' => ['application/json' => [
                    'schema' => ['type' => 'object', 'required' => ['code','name','kind','unit'], 'properties' => [
                        ...
                        'kind' => ['type' => 'string', 'enum' => ['raw','finished'], 'example' => 'raw'],
                        'unit' => ['type' => 'string', 'example' => 'g'],
                        'status' => ['type' => 'string', 'example' => 'active'],
                        'metadata' => ['type' => 'object', 'nullable' => true],
                    ]],
                    'example' => ['code' => 'COFFEE_BEANS', 'name' => 'Coffee Beans', 'kind' => 'raw', 'unit' => 'g'],
                ]],
            ],
